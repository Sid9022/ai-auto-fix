...
export default function History() {
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8 animate-fade-in">
      <div className="mb-6 sm:mb-8 animate-slide-in-left">
        ...
        <p className="text-sm sm:text-base text-muted-foreground animate-fade-in animate-delay-200">
          View your past vehicle diagnoses. Records auto-delete after 7 days unless favorited.
        </p>
      </div>

      <div className="mb-4 sm:mb-6 animate-slide-in-right animate-delay-300">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground h-4 w-4 animate-pulse-slow" />
          <Input
            placeholder="Search your diagnostic history..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="pl-10 text-sm sm:text-base glass border-0 transition-all duration-300 focus:scale-[1.02] focus:shadow-glow"
          />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Card className="glass border-0 shadow-soft animate-bounce-in">
          <CardContent className="py-12 text-center">
            <Clock className="h-12 w-12 text-muted-foreground mx-auto mb-4 animate-pulse-slow" />
          ...
          {filteredHistory.map((item, index) => {
            ...
            return (
              <Card 
                key={item.id} 
                className={`
                  transition-all duration-500 hover-lift glass border-0 shadow-soft
                  animate-fade-in animate-delay-${Math.min(index * 100, 500)}
                ...
                <CardHeader className="pb-3 sm:pb-6">
                  <div className="space-y-3 sm:space-y-0 sm:flex sm:items-start sm:justify-between">
                    <div className="flex-1 min-w-0">
                      <CardTitle className="text-base sm:text-lg flex flex-col sm:flex-row sm:items-center gap-2 animate-bounce-in">
                        <span className="truncate pr-2">{item.predicted_fault}</span>
                        <Badge variant={getSeverityColor(item.severity)} className="self-start sm:self-center animate-scale-in animate-delay-200">
                          {item.severity}
                        </Badge>
                      </CardTitle>
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-xs sm:text-sm text-muted-foreground mt-2 animate-fade-in animate-delay-300">
                        <div className="flex items-center gap-1">
                          <Calendar className="h-3 w-3 sm:h-4 sm:w-4 animate-pulse-slow" />
                        ...
                        {daysRemaining !== null && (
                          <div className="flex items-center gap-1">
                            <Clock className="h-3 w-3 sm:h-4 sm:w-4 animate-pulse-slow" />
                    ...
                    <div className="flex items-center gap-1 sm:gap-2 ml-auto sm:ml-0 animate-slide-in-right animate-delay-400">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => toggleFavorite(item.id, !item.is_favorite)}
                        className="h-8 w-8 sm:h-9 sm:w-9 p-0 hover-scale"
                      >
                        <Star className={`h-3 w-3 sm:h-4 sm:w-4 ${item.is_favorite ? 'fill-yellow-400 text-yellow-400 animate-bounce-in' : ''}`} />
                      </Button>
                      <Button
                        variant="ghost"
                        size="sm"
                      ...
                      <Button
                        ...
                        className="h-8 w-8 sm:h-9 sm:w-9 p-0 hover-scale text-destructive hover:text-destructive"
                      >
                ...
                <CardContent className="pt-0">
                  <div className="space-y-3">
                    ...
                    <Button
                      ...
                      className="p-0 h-auto text-primary hover:no-underline text-xs sm:text-sm story-link hover-scale animate-fade-in animate-delay-600"
                    >
                      {isExpanded ? 'Show less' : 'Show details'}
                    </Button>
                    
                    {isExpanded && (
                      <div className="space-y-3 sm:space-y-4 pt-3 border-t animate-accordion-down">
                          ...
                          <div className="animate-slide-in-left animate-delay-100">
                            ...
                            <ul className="list-disc list-inside text-xs sm:text-sm text-muted-foreground mt-1 space-y-1 ml-2 sm:ml-0">
                              {item.recommended_actions.map((action, index) => (
                                <li key={index} className={`leading-relaxed animate-fade-in animate-delay-${200 + index * 100}`}>{action}</li>
                              ))}
                            </ul>
                          </div>
                        )}
                          ...
                          <div className="animate-slide-in-left animate-delay-200">
                            ...
                            <div className="mt-1 space-y-2">
                              {item.alternatives.map((alt: any, index: number) => (
                                <div key={index} className={`flex items-start gap-2 text-xs sm:text-sm glass p-2 rounded hover-scale animate-fade-in animate-delay-${300 + index * 100}`}>
